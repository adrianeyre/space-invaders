...
export default class Game implements IGame {
	...
	public handleInput = (playerResult: PlayerResultEnum, sprite?: ISprite): void => {
		switch (playerResult) {
			...
			case PlayerResultEnum.SPRITE_DEAD:
				this.killSprite(sprite ? sprite : undefined);
				break;
			case PlayerResultEnum.ALIEN1_POINTS:
				this.player.addScore(this.SCORE_ALIEN1);
				break;
			case PlayerResultEnum.ALIEN2_POINTS:
				this.player.addScore(this.SCORE_ALIEN2);
				break;
			case PlayerResultEnum.ALIEN3_POINTS:
				this.player.addScore(this.SCORE_ALIEN3);
				break;
			case PlayerResultEnum.ALIEN4_POINTS:
				this.player.addScore(this.SCORE_ALIEN4);
				break;
			case PlayerResultEnum.ARROW_RIGHT:
				this.movePlayer(DirectionEnum.RIGHT);
				break;
			case PlayerResultEnum.ARROW_LEFT:
				this.movePlayer(DirectionEnum.LEFT);
				break;
	...
	public handleTimer = (): void => {
		...
		if (
			furthestRightAlien &&
			this.direction === DirectionEnum.RIGHT &&
			furthestRightAlien.x + furthestRightAlien.width > this.SPRITE_BLOCKS_WIDTH
		) {
		...
		if (
			...
			furthestLeftAlien.x + furthestLeftAlien.width < furthestLeftAlien.width
		) {
			this.alienTimerInterval -= this.DEFAULT_ALIEN_TIMER_DECREMENT;
			this.direction = DirectionEnum.DOWN;
		...
		if (Math.floor(Math.random() * 100) > 60) {
			...
			aliveAliens.forEach((alien: ISprite) => {
				if (
					aliveAliens.filter(
						(findAlien: ISprite) => alien.key !== findAlien.key && findAlien.y > alien.y && findAlien.x === alien.x
					).length === 0
				) {
	...
	private getPlayerBullet = (): ISprite | undefined =>
		this.sprites.find((sprite: ISprite) => sprite.type === SpriteTypeEnum.PLAYER_BULLET);
	private getAlienBullet = (): ISprite[] =>
		this.sprites.filter((sprite: ISprite) => sprite.type === SpriteTypeEnum.ALIEN_BULLET);
	private resetSprites = (): ISprite[] =>
		(this.sprites = spritesData.default.map((sprite: ISpriteProps) => new Sprite(sprite)));

	private aliveAliens = (): ISprite[] =>
		this.sprites.filter(
			(sprite: ISprite) =>
				sprite.visable &&
