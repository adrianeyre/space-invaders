...
import type ISpaceInvadersState from './interfaces/space-invaders-state';
...
export default class SpaceInvaders extends React.Component<ISpaceInvadersProps, ISpaceInvadersState> {
	...
	public render() {
		return (
			<div
				className="space-invaders-play-container"
				ref={(d) => {
					this.container = d;
				}}
			...
			>
				...
				<div style={this.styleStatusBottom()}>
					<GameStatusBottom lives={this.state.game.player.lives - 1} level={this.state.game.level} />
				</div>
	...
	private handleTimerUpdates = () => {
		if (
			this.state.alienTimerInterval === this.state.game.alienTimerInterval &&
			this.state.bulletTimerInterval === this.state.game.bulletTimerInterval
		)
			return;
